...
def consultar_parametro(dado_1, dado_2, dado_3):
    ...
    parametro_cadastrado = session.query(dado_1).filter(
        dado_2 == dado_3
    ).first()
    session.close()
...
def consultar_dados_gerais_banco(database, coluna_filtro, valor, coluna_ordem):
    ...
    tuplas = (
        ...
        .filter(coluna_filtro == valor)
        .order_by(asc(coluna_ordem))
        .all()
    )
    session.close()
...
def obter_valor_total(coluna_soma, coluna_1, valor_1):
    ...
    return 0 if valor_total is None else valor_total


def somar_condicional(coluna_soma, coluna_1, valor_1, coluna_2, valor_2):
    session = session_maker()
    valor_total = (
        session.query(func.sum(coluna_soma))
        .filter(coluna_1 == valor_1, coluna_2 == valor_2)
        .scalar()
    )
    session.close()
...
def atualizar_banco(
    database, coluna_1, valor_1, coluna_2, valor_2, coluna_update, novo_valor
):
    session = session_maker()
    session.query(database).filter(
        coluna_1 == valor_1, coluna_2 == valor_2
    ).update(
        {coluna_update: novo_valor}
    )
    session.commit()
    session.close()


def consultar_parametro_secao(
    coluna_procurada, coluna_1, valor_1, coluna_2, valor_2
):
    session = session_maker()
    parametro_cadastrado = (
        session.query(coluna_procurada)
        .filter(coluna_1 == valor_1, coluna_2 == valor_2)
        .first()
    )
    session.close()
