...
class VendaJoinAutomovelSchema(BaseModel):
    """ Define nova estrutura que será apresentada quando realizado o join entre as tabelas vendas e automóvel.
    """
    proprietario: str = 'João'
    valor: float = 90000.00
    ano: int = 2020
...
def apresentar_venda(dados_venda: Vendas, mensagem):
    """ Define um modelo de resposta para quando uma venda for cadastrada à base de dados.
    """
    result = {
...
def listar_vendas_all(lista_vendas: list):
    """ Define um modelo de resposta para quando for solicitado uma lista dos vendas cadastradas na base de dados.
    """
    result = []
