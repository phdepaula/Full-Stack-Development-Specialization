...
def cadastrar_automovel(form: AutomovelSchema):
    ...
    automovel = Automovel(
        ...
        marca=form.marca.capitalize(),
        tipo=form.tipo.capitalize(),
    )
...
def deletar_automovel(query: AutomovelBuscaSchema):
    ...
    try:
        ...
        venda_cadastrada = (
            session.query(Vendas.id_vendas)
            .filter(
                Vendas.id_automovel
                == (
                    session.query(Automovel.id_automovel)
                    .filter(Automovel.nome == nome_automovel.capitalize())
                    .first()[0]
                )
            )
            .first()
...
def cadastrar_venda(form: VendaSchema):
    ...
    try:
        ...
        id_automovel = (
            session.query(Automovel.id_automovel)
            .filter(Automovel.nome == nome_automovel)
            .first()
        )
        ...
        if id_automovel == None:
            ...
            venda = Vendas(
                proprietario=form.proprietario.capitalize(),
